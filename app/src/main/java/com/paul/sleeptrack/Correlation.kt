...
@Composable
private fun Scatter(pairs: List<DayNight>, c: SleepColors) {
    ...
    Column {
        Canvas(Modifier.fillMaxWidth().height(150.dp)) {
            ...
            while (hour <= maxSleep) {
                ...
                drawLine(c.outline, Offset(0f, y), Offset(w, y), strokeWidth = 1f)
                hour++
            }
            for (p in pairs) {
                ...
                drawCircle(
                    color = scaleForSleepHours(p.sleepHours, c.levels).copy(alpha = 0.75f),
                    radius = 3.dp.toPx(),
...
private fun scaleForSleepHours(hours: Double, levels: List<Color>): Color =
    Scale(listOf(300.0, 360.0, 420.0, 480.0), false, levels, emptyList()).colorOf(hours * 60)
